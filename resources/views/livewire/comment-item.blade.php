<div>
    <div class="flex justify-between mb-4 gap-3">
        ...
        <div class="flex-1">
            ...
            <div>
                ...
                @if (\Illuminate\Support\Facades\Auth::id() == $comment->user_id)
                    ...
                    <a wire:click.prevent="deleteComment" href="#" class="text-sm text-red-600">Delete
                @endif
                </a>
            </div>
        </div>
        ...
        @if (!empty($comment->comments))
            <div class="mt-4">
                @foreach ($comment->comments as $childComp)
                    <livewire:comment-item :comment="$childComp" wire:key="comment-{{ $childComp->id }}" />
                @endforeach
            </div>
        @endif
    </div>
